...
### Local Notes
This repo is `@plasius/gpu-shader`, the shared reflection-first WebGPU WGSL
interface, compatibility, runtime style-profile, and qualification package.
...
- The root export must remain browser-safe. Node reflection and generation belong
  in `@plasius/gpu-shader/node`; qualification helpers belong in
  `@plasius/gpu-shader/testing`.
- Final assembled WGSL is the only source of truth for host-shareable GPU record
  layout. Sidecars may select reflected records and semantics, but must never
  supply offsets, alignment, sizes, or strides.
- Add tests for new or changed TypeScript behavior and keep framework line
  coverage at 80% or higher.
- WGSL is exempt from line coverage only when every WGSL fragment belongs to at
  least one compile unit and every required matrix cell passes.
- Qualification candidates are data-only (`.json`, `.wgsl`, bounded `.bin`).
  Physical runners must execute the trusted harness from this repository and
  must never execute candidate JavaScript, shell code, or callbacks.
...
- Do not describe a shader as universally supported unless every declared
  physical stable-WebGPU matrix cell has current passing evidence. SwiftShader
  is blocking smoke coverage, not physical support evidence.
- Missing runners, adapters, results, timeouts, skips, or device loss fail the
  qualification gate. Hosted fallbacks must not substitute for physical cells.
- Only the approved `cd.yml` workflow on `main`, using the `production`
  environment, may publish the package.
