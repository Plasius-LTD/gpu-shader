...
## 18. WGSL Shader Framework Requirements

These package-specific requirements refine the general rules above.

### Correctness and reliability (Critical)

- Final assembled WGSL is the only source of host-shareable GPU record layout.
- CPU codecs must be deterministic, bounds checked, little-endian, and write
  padding deterministically.
- Storage admission and runtime compatibility both fail closed.
- Disabling `asset.pipeline.shader-store.enabled` blocks public candidate
  submission and runtime discovery/loading/activation without blocking
  separately authorized private qualification, immutable promotion or rollback
  rehearsal; it never authorizes unmanaged fallback shaders.
- Style replacement must preserve the active/default profile until every
  replacement module and pipeline has prepared successfully and a frame
  boundary activation completes.
- Manifest and evidence parsing must reject unknown fields, stale identities,
  incomplete products, non-finite numbers and ambiguous references.

### Supply-chain and runner isolation (Critical)

- Candidate qualification archives are data-only and bounded.
- Physical runner hosts execute only the pinned trusted harness; candidate
  JavaScript, native code, shell commands, callbacks and browser extensions are
  prohibited.
- Candidate jobs receive no npm publication, release-app, runner-management or
  long-lived cloud credentials.
- Every manifest, module, fixture, inventory, matrix, harness and evidence
  subject uses an exact SHA-256 identity.

### Compatibility evidence (Critical)

- Every WGSL fragment belongs to at least one final compile unit.
- Every required compile-unit-by-matrix pair produces exactly one passing
  result for a promotable attempt.
- Missing runners/adapters/results, skips, timeouts and device loss are
  failures. Physical cells have no hosted fallback.
- SwiftShader is required smoke coverage but cannot satisfy physical support.
- Evidence records actual browser, OS, adapter, backend, driver, features,
  limits, runner, toolchain and workflow provenance.

### Performance (High)

- Browser consumers import the root package without eagerly loading Node
  reflection or browser automation dependencies.
- Profile preparation is asynchronous and does not block frame-boundary
  activation work with network fetch, parsing or shader compilation.
- Catalog loading has bounded concurrency and honors cancellation.
- Qualification resource sizes, command counts and execution time are bounded.

### Testability and maintainability (High)

- Framework TypeScript maintains at least 80% LCOV line coverage and every
  changed source file appears in combined LCOV.
- WGSL uses 100% fragment inventory plus 100% required matrix passage instead
  of line coverage.
- Public contracts are versioned and deterministic; breaking changes follow
  SemVer and include migration notes and requalification impact.
