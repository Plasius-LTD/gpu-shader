...
test("rejects CLA registries across platforms and package roots", () => {
  const violations = findPrivateArtifactViolations([
    ...
    "legal/contributors-registry/acceptance.pdf",
  ]);
  ...
  assert.deepEqual(
    ...
    [
      ...
      {
        artifactPath: "legal/contributors-registry/acceptance.pdf",
        ruleId: "contributor-registry",
      },
      {
...
test("requires narrow, explicit package files entries", () => {
  assert.deepEqual(
    findPackageFilesPolicyViolations([], ["dist", "matrices"]).map(
  ...
  for (const broadEntry of [".", "*", "**/*", "legal"]) {
    assert.deepEqual(
      findPackageFilesPolicyViolations(
        ["dist", "matrices", broadEntry],
        ["dist", "matrices"]
      ).map(({ entry, ruleId }) => ({ entry, ruleId })),
...
test("repository discovery fails closed for invalid Git metadata", (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "private-artifact-git-"));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));
  ...
  assert.throws(
    ...
    /Could not determine whether the policy root is a Git worktree\./u
  );
});

test("repository discovery fails closed outside a Git worktree", (t) => {
...
test("repository discovery fails closed when the Git index is unreadable", (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "private-artifact-index-"));
  t.after(() => fs.rmSync(root, { recursive: true, force: true }));

  const init = spawnSync("git", ["init", "--quiet", root], { encoding: "utf8" });
  assert.equal(init.status, 0, init.stderr);
  fs.writeFileSync(path.join(root, ".git", "index"), "invalid index", "utf8");

  assert.throws(
    () => collectRepositoryArtifactPaths(root),
    /Could not enumerate Git index paths for private-artifact policy\./u
  );
});
