...
## 9. Shader Framework Guidance
- The shader-store Feature uses the remotely controlled flag
  `asset.pipeline.shader-store.enabled` for public candidate submission and
  runtime shader/profile discovery, loading, and activation.
- Separately authorized private intake, qualification, immutable promotion, and
  rollback rehearsal remain available while the flag is disabled.
- Disabled behavior fails closed and does not authorize checked-in catalogs,
  arbitrary Blob URLs, unmanaged shaders, or a general legacy fallback. A
  separately governed bounded pre-store default is permitted only during the
  documented migration window.
- User-visible compatible-style discovery and selection uses capability
  `gpu.shader.style.select` in addition to the Feature flag.
- Rendering the model's promoted default style does not require the
  style-selection capability.
- This package defines portable contracts and behavior. The asset service owns
  feature-flag evaluation and the application/capability service owns user
  entitlement decisions.
- Tests must cover disabled public submission/runtime access, authorized private
  operation while disabled, enabled store access, catalog rollback, rejection
  of unmanaged fallback, style-selection denied, style-selection granted, and
  promoted-default rendering without style-selection access.
