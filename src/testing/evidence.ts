...
function sha(value: unknown, path: string): Sha256Hex {
  ...
  catch (cause) { throw new TypeError(`${path} is not a SHA-256 digest.`, { cause }); }
}
...
export function parseTrustedWorkflowProvenance(value: unknown, path = "provenance"): ShaderTrustedWorkflowProvenance {
  const input = object(value, path); exact(input, ["repository", "commit", "ref", "workflowRef", "workflowSha", "runId", "runAttempt", "job", "eventName", "oidcAttestation"], path);
  ...
  return input as unknown as ShaderTrustedWorkflowProvenance;
}
...
export function parseQualificationExecutionProducer(value: unknown, path = "producer"): ShaderQualificationExecutionProducer {
  const input = object(value, path); exact(input, ["repository", "commit", "ref", "runId", "runAttempt", "job", "trustedWorkflowRepository", "trustedWorkflowRef", "trustedWorkflowSha", "runner"], path);
  ...
  return input as unknown as ShaderQualificationExecutionProducer;
}
...
export async function computeQualificationInventorySha256(bundle: ShaderQualificationBundleManifest): Promise<Sha256Hex> {
  return computeSha256(canonicalizeGpuContract(bundle.inventory));
}
...
export async function createQualificationPreflight(input: {
  ...
  catch (cause) { throw new TypeError("Selected matrix artifact is not UTF-8 JSON.", { cause }); }
...
export async function aggregateShaderValidationEvidence(input: AggregateShaderValidationEvidenceInput): Promise<ShaderValidationEvidence> {
  const verifyArtifact = async <T>(artifact: EvidenceArtifact<T>, label: string): Promise<void> => {
    ...
    catch (cause) { throw new TypeError(`${label} bytes are not UTF-8 JSON.`, { cause }); }
  ...
  for (const cell of input.matrix.cells) {
    ...
    if (runner?.value.status === "runner-unavailable") {
      ...
      const cellResults = results.slice(-units.size); cellRuns.push({ cellId: cell.cellId, source: "runner-preflight", sourceArtifactSha256: runner.sha256, resultsSha256: await computeSha256(canonicalizeGpuContract(cellResults)), qualificationPreflightProvenance: preflight.provenance, producer: runner.value.producer, harness: input.harness, automation: null, resultCount: units.size, status: "runner-unavailable" });
      continue;
    }
    if (diagnostic) {
      ...
      const cellResults = results.slice(-units.size); cellRuns.push({ cellId: cell.cellId, source: "workflow-diagnostic", sourceArtifactSha256: diagnostic.sha256, resultsSha256: await computeSha256(canonicalizeGpuContract(cellResults)), qualificationPreflightProvenance: preflight.provenance, producer: value.producer, harness: input.harness, automation: null, resultCount: units.size, status });
      continue;
    }
...
export async function validateShaderValidationEvidence(input: {
  ...
  try {
    const evidence = object(input.evidence, "evidence"); exact(evidence, ["contractVersion", "evidenceId", "status", "generatedAt", "subjectBindingSha256", "subject", "matrixRef", "toolchain", "qualificationPreflightProvenance", "counts", "cellRuns", "results"], "evidence");
    ...
    let parsedMatrix: unknown; try { parsedMatrix = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(input.matrixBytes)); } catch (cause) { throw new TypeError("Matrix artifact is not UTF-8 JSON.", { cause }); }
    if (!canonicalEqual(parsedMatrix, input.matrix)) throw new TypeError("Matrix artifact bytes differ from the supplied matrix value.");
    const matrixSha256 = await computeSha256(input.matrixBytes);
    const inventorySha256 = await computeQualificationInventorySha256(input.bundle);
    if (inventorySha256 !== input.bundle.subject.compileUnitInventorySha256) throw new TypeError("Bundle subject inventory digest is stale.");

    const subject = object(evidence.subject, "evidence.subject"); const expectedSubjectKeys = [...Object.keys(input.bundle.subject), "dataBundleSha256"]; exact(subject, expectedSubjectKeys, "evidence.subject");
    const candidateSubject = Object.fromEntries(Object.entries(subject).filter(([key]) => key !== "dataBundleSha256")); if (!canonicalEqual(candidateSubject, input.bundle.subject)) throw new TypeError("Evidence candidate subject differs from bundle.");
    ...
    const matrixRef = object(evidence.matrixRef, "evidence.matrixRef"); exact(matrixRef, ["matrixId", "version", "sha256"], "evidence.matrixRef"); if (matrixRef.matrixId !== input.matrix.matrixId || matrixRef.version !== input.matrix.version || matrixRef.sha256 !== matrixSha256) throw new TypeError("Evidence matrix artifact identity differs.");
    ...
    const expectedBinding = await computeQualificationSubjectBinding({ subject: input.bundle.subject, dataBundleSha256, compileUnitInventorySha256: inventorySha256, matrixSha256, harnessSha256 }); if (subjectBindingSha256 !== expectedBinding) throw new TypeError("Evidence subject binding is stale.");
    if (evidenceId !== await computeValidationEvidenceId(evidence)) throw new TypeError("Evidence identity is stale.");

    const units = new Set(input.bundle.inventory.compileUnits.map((unit) => unit.compileUnitId)); const cells = new Map(input.matrix.cells.map((cell) => [cell.cellId, cell])); const expected = units.size * cells.size;
    ...
    for (const [index, value] of runs.entries()) {
      ...
      const cellResults = parsedResults.filter((result) => result.cellId === cellId); if (run.resultCount !== units.size || cellResults.length !== units.size || await computeSha256(canonicalizeGpuContract(cellResults)) !== resultsSha256) throw new TypeError(`${path} result count/digest is stale.`);
    ...
    return { ok: true, value: evidence as unknown as ShaderValidationEvidence };
  } catch (cause) {
...
export function parseShaderValidationEvidenceAttestationRef(value: unknown): ShaderValidationEvidenceAttestationRef {
  const input = object(value, "attestationRef"); exact(input, ["contractVersion", "kind", "evidence", "attestation", "producer"], "attestationRef"); if (input.contractVersion !== SHADER_VALIDATION_EVIDENCE_VERSION || input.kind !== "shader-validation-evidence-attestation-ref") throw new TypeError("Unsupported evidence attestation ref.");
...
export async function verifyShaderValidationEvidenceAttestation(input: {
  ...
  try {
    const ref = parseShaderValidationEvidenceAttestationRef(input.ref); if (await computeSha256(input.evidenceBytes) !== ref.evidence.sha256 || await computeSha256(input.bundleBytes) !== ref.attestation.bundle.sha256) throw new TypeError("Attested evidence or bundle digest differs."); if (!await input.verifyCryptographicBundle({ ref, evidenceBytes: input.evidenceBytes, bundleBytes: input.bundleBytes })) throw new TypeError("External build-provenance cryptographic verification failed."); return { ok: true, value: ref };
